...
import {
  ...
  BarChart3, TrendingUp, Zap, AlertTriangle, Loader2
} from 'lucide-react';
import { useRepos, useRegistryItems } from '../api/hooks';
...
export function Dashboard() {
  ...
  const { data: registryData } = useRegistryItems();

  const repositories = reposData?.data || reposData || [];
  const registryItems = registryData?.data || registryData || [];
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-4">
        ...
        <div className="flex bg-[#161b22] border border-[#30363d] p-1 rounded-sm divide-x divide-[#30363d]">
          <div className="px-4 py-2 text-center">
            ...
            <div className="text-xl font-display text-white">{reposLoading ? '...' : repositories.length}</div>
          </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-8">
        ...
        <div className="xl:col-span-2 space-y-6">
          ...
          {reposLoading ? (
            <div className="flex items-center justify-center py-12">
              ...
              <span className="ml-3 text-gray-400 text-sm font-mono">Loading repositories...</span>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {repositories.map((repo: any) => (
                <Link
                  key={repo.id}
                  to={`/${repo.owner_name || 'user'}/${repo.name}`}
                ...
                >
                  ...
                  <div className="mt-6 flex flex-wrap gap-2">
                    <span className="text-[9px] font-black uppercase tracking-widest bg-blue-500/10 text-blue-500 px-2 py-0.5 border border-blue-500/20">
                      {repo.language || 'General'}
                    </span>
                    <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-0.5 border ${repo.is_private ? 'bg-red-500/10 text-red-500 border-red-500/20' : 'bg-green-500/10 text-green-500 border-green-500/20'}`}>
                  ...
                  <div className="mt-4 pt-4 border-t border-gray-100/5 flex items-center justify-between text-[10px] font-mono">
                    ...
                    <span className="text-gray-600">
                      {repo.updated_at ? formatDistanceToNow(new Date(repo.updated_at)) + ' ago' : 'Just now'}
                    </span>
                  </div>
                </Link>
              ))}
              ...
              {repositories.length === 0 && (
                <div className="col-span-full p-8 text-center">
                  <div className="text-gray-500 font-mono text-sm">No repositories yet.</div>
                  <Link to="/workspace" className="text-orange-500 text-sm font-bold hover:underline mt-2 inline-block">
        ...
        <div className="xl:col-span-1 space-y-6">
          <div className="industrial-card p-6 border-l-4 border-blue-500">
            ...
            <div className="space-y-4">
              {registryItems.slice(0, 4).map((item: any) => (
                <div key={item.id} className="flex items-center justify-between p-3 bg-gray-800/50 border border-gray-700/50 rounded-sm">
                  <div className="flex items-center space-x-3">
                    <div className={`p-2 rounded bg-opacity-10 ${item.type === 'agent' ? 'bg-purple-500 text-purple-500' : item.type === 'cli' ? 'bg-blue-500 text-blue-500' : 'bg-orange-500 text-orange-500'}`}>
              ...
              {registryItems.length === 0 && (
                <div className="text-center py-4 text-gray-500 text-xs font-mono">No toolkit items yet.</div>
              )}
            </div>
          </div>
